...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(await instance.name.call(), 'Rithvik Token');
    assert.equal(await instance.symbol.call(), 'RTV');
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let name1 = await instance.lookUptokenIdToStarInfo(tokenId, {from:user1});
    // 3. Verify if you Star name is the same
    assert.equal(name1, 'awesome Star10');
});